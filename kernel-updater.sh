...
UPDATE_NEEDED="$("$LOCALDIR"/kernel-updater.py --check-update --base "$GIT_UPSTREAM:${BRANCH_linux_kernel:-master}")"
if [ -n "$UPDATE_NEEDED" ]; then
    git clone -b "${BRANCH_linux_kernel}" "${GIT_BASEURL_UPSTREAM}/${GIT_PREFIX_UPSTREAM}linux-kernel" "$KERNELDIR"

    # For keys only
    git clone "${GIT_BASEURL_UPSTREAM}/${GIT_PREFIX_UPSTREAM}builder-rpm" "$BUILDDIR/builder-rpm"
    ...
    if [ -n "$(git diff version)" ]; then
        ...
        "$LOCALDIR/kernel-updater.py" --create-pullrequest --base "$GIT_UPSTREAM:${BRANCH_linux_kernel:-master}" --head "$GIT_FORK:$HEAD_BRANCH"
    fi
fi